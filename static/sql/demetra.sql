select
'reservation id;creation date;arrival;departure;rate amount;nights;shource code;group;block;status;category;rate code;adult number;no of rooms;cancellation date;packages;market;company agent;rates;br rates;accrev;fbrev;otherrev;tlresnum' header
from
dual
union all
select
...
FROM
YRES
where yres_id not in (select ydet_yres_id from ydet, yres r where ydet_ygrp_id is not null and ydet_yres_id = r.yres_id and r.yres_resstatus = 0)) ri