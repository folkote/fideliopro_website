...
select
    ...
    || ';' || nvl((select sum(noofrooms) from m_ydet where ydet_date = wdat.wdat_date
        and ydet_ycat_id = ycat.ycat_id and nvl(ydet_ygrp_id, 0) > 0), 0)
    || ';' || 0
    || ';' || nvl((select sum(zpos_grossunitprice * zpos_quantity) from (select zpos_postdate, zpos_originated_yres_id, zpos_grossunitprice, zpos_quantity, zpos_zdco_id, zpos_manualpost from zpos union all select ypos_date, ypos_yres_id, ypos_grossunitprice, ypos_quantity, ypos_zdco_id, 0 from ypos) zpos, zdco, yres, ydet where zpos_postdate = wdat.wdat_date
    ...
    || ';' || nvl((select sum(zpos_grossunitprice * zpos_quantity) from (select zpos_postdate, zpos_originated_yres_id, zpos_grossunitprice, zpos_quantity, zpos_zdco_id, zpos_manualpost from zpos union all select ypos_date, ypos_yres_id, ypos_grossunitprice, ypos_quantity, ypos_zdco_id, 0 from ypos) zpos, zdco, yres, ydet where zpos_postdate = wdat.wdat_date
        ...
        and zdco_cdt = 1 and nvl(ydet_ygrp_id, 0) > 0), 0)
from wdat, ycat
where nvl(ycat_disabled, 0) = 0
  and wdat_date between least(
        nvl((select min(trunc(ydet_date)) from ydet), trunc(v8_sys_fideliodate)),
        nvl((select min(trunc(ybld_date)) from ybld), trunc(v8_sys_fideliodate)),
        nvl((select min(trunc(zpos_postdate)) from zpos), trunc(v8_sys_fideliodate)),
        nvl((select min(trunc(yrst_fromtime)) from yrst), trunc(v8_sys_fideliodate))
      )
      and add_months(trunc(v8_sys_fideliodate), 12)
