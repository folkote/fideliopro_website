select
'reservation id;creation date;arrival;departure;rate amount;nights;shource code;group;block;status;category;rate code;adult number;no of rooms;cancellation date;packages;market;company agent;rates;br rates;accrev;fbrev;otherrev;tlresnum;sharenum' header
from
dual
union all
select
...
|| ';' || to_char((select avg(ypal_value * ypal_multiplier) from ydet,ypal,ypac where
...
|| ';' || aggregate_field('select sum(ypal_value * ypal_multiplier)
                     from ydet,ypal,ypac where ypal_ydet_id = ydet_id and ypal_ypac_id = ypac_id and ydet_yres_id = '
                      || to_char(yres_id) || ' group by ydet_date order by ydet_date asc', '/')
|| ';' || accrev
|| ';' || fbrev
|| ';' || otherrev
|| ';' || tlresnum
...
from
  ...
  (SELECT XCMA.XCMA_SHORTDESC FROM XCMA,YDET WHERE XCMA.XCMA_ID=YDET.YDET_XCMA_ID AND YDET.YDET_ID=YRES.YRES_CURRENT_YDET_ID) XCMA_SHORTDESC,
   (select trim(trim(xcms_name1 || ' ' || xcms_name2) || ' ' || xcms_name3) from ycln, xcms
    ...
    , (select sum(zpos_grossunitprice * zpos_quantity) from zpos, zdco where zpos_cdt = 1 and
        zpos_originated_yres_id = yres.yres_id and zpos_zdco_id = zdco_id and zdco_stats_type = 1) accrev
    , (select sum(zpos_grossunitprice * zpos_quantity) from zpos, zdco where zpos_cdt = 1 and
        zpos_originated_yres_id = yres.yres_id and zpos_zdco_id = zdco_id and zdco_stats_type = 2) fbrev
    , (select sum(zpos_grossunitprice * zpos_quantity) from zpos, zdco where zpos_cdt = 1 and
        zpos_originated_yres_id = yres.yres_id and zpos_zdco_id = zdco_id and zdco_stats_type not in (1, 2)) otherrev
        , yres.yres_crsresnr tlresnum
    , (select ydet_sharenum from ydet where ydet_id = yres.yres_last_ydet_id) sharenum
FROM
YRES) ri
where yres_expdeptime >= add_months(trunc(v8_sys_fideliodate), -1)
  and yres_expdeptime <= add_months(trunc(v8_sys_fideliodate), 12)
